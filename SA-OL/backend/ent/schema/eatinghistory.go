...
func (Eatinghistory) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("mealplan", Mealplan.Type).Ref("eatinghistorys").Unique(),
		edge.From("foodmenu", Foodmenu.Type).Ref("eatinghistorys").Unique(),
		edge.From("taste", Taste.Type).Ref("eatinghistorys").Unique(),
		edge.From("user", User.Type).Ref("eatinghistorys").Unique(),
	}
}
