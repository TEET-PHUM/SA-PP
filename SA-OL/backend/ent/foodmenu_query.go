...
func (fq *FoodmenuQuery) QueryEatinghistorys() *EatinghistoryQuery {
...
func (fq *FoodmenuQuery) WithEatinghistorys(opts ...func(*EatinghistoryQuery)) *FoodmenuQuery {
	query := &EatinghistoryQuery{config: fq.config}
	for _, opt := range opts {
		opt(query)
	}
	fq.withEatinghistorys = query
	return fq
}
...
func (fq *FoodmenuQuery) sqlAll(ctx context.Context) ([]*Foodmenu, error) {
	...
	if query := fq.withEatinghistorys; query != nil {
		...
		query.Where(predicate.Eatinghistory(func(s *sql.Selector) {
			s.Where(sql.InValues(foodmenu.EatinghistorysColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
