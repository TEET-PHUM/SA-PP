...
func (mq *MealplanQuery) QueryEatinghistorys() *EatinghistoryQuery {
...
func (mq *MealplanQuery) WithEatinghistorys(opts ...func(*EatinghistoryQuery)) *MealplanQuery {
	query := &EatinghistoryQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withEatinghistorys = query
	return mq
}
...
func (mq *MealplanQuery) sqlAll(ctx context.Context) ([]*Mealplan, error) {
	...
	if query := mq.withEatinghistorys; query != nil {
		...
		query.Where(predicate.Eatinghistory(func(s *sql.Selector) {
			s.Where(sql.InValues(mealplan.EatinghistorysColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
