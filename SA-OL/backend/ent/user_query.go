...
type UserQuery struct {
	...
	// eager-loading edges.
	withFoodmenus      *FoodmenuQuery
	withMealplans      *MealplanQuery
	withEatinghistorys *EatinghistoryQuery
...
// QueryFoodmenus chains the current query on the foodmenus edge.
func (uq *UserQuery) QueryFoodmenus() *FoodmenuQuery {
...
// QueryEatinghistorys chains the current query on the eatinghistorys edge.
func (uq *UserQuery) QueryEatinghistorys() *EatinghistoryQuery {
...
//  WithFoodmenus tells the query-builder to eager-loads the nodes that are connected to
// the "foodmenus" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithFoodmenus(opts ...func(*FoodmenuQuery)) *UserQuery {
	query := &FoodmenuQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withFoodmenus = query
	return uq
}
...
//  WithEatinghistorys tells the query-builder to eager-loads the nodes that are connected to
// the "eatinghistorys" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithEatinghistorys(opts ...func(*EatinghistoryQuery)) *UserQuery {
	query := &EatinghistoryQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withEatinghistorys = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [3]bool{
			uq.withFoodmenus != nil,
			uq.withMealplans != nil,
			uq.withEatinghistorys != nil,
		}
	)
	...
	if query := uq.withFoodmenus; query != nil {
		...
		query.Where(predicate.Foodmenu(func(s *sql.Selector) {
			s.Where(sql.InValues(user.FoodmenusColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Foodmenus = append(node.Edges.Foodmenus, n)
		}
	}
	...
	if query := uq.withEatinghistorys; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Eatinghistory(func(s *sql.Selector) {
			s.Where(sql.InValues(user.EatinghistorysColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Eatinghistorys = append(node.Edges.Eatinghistorys, n)
		}
	}
